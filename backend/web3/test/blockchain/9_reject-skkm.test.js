...
describe("Unverify Activity Credit in the Activity Credit Contract", function () {
    ...
    beforeEach(async function () {
        ...
        // Submitting five Activity Credits for unverification testing
        for (let i = 1; i <= 5; i++) {
    ...
    // Unverify five existing Activity Credits by the Departmental Student Association
    it("should unverify five existing Activity Credits by the Departmental Student Association", async function () {
        this.timeout(120000); // Set timeout to 120 seconds
        ...
        for (let i = 0; i < 5; i++) {
            try {
                // Unverifying Activity Credit
                console.log(`Unverifying Activity Credit ${i + 1}...`);
                startTime = process.hrtime(); // Record time before transaction
                tx = await skkm.connect(departmentStudentAssociation).verifySKKM(i, false, "Unverified by Departmental Student Association"); // Unverify Activity Credit with corresponding ID
                ...
                console.log(`Unverify Activity Credit ${i + 1} transaction hash: ${receipt.transactionHash}`);
                console.log(`Block number: ${receipt.blockNumber}`);
                console.log(`Transaction time for Activity Credit ${i + 1}: ${formattedTime} seconds`);

                // Verify that the Activity Credit has been unverifiable
                const skkmRequest = await skkm.requests(i);
                expect(skkmRequest.status).to.equal(2); // Status.Unverified
            } catch (error) {
                console.error(`Failed to unverify Activity Credit ${i + 1}:`, error);
                throw new Error(`Failed to unverify Activity Credit ${i + 1}`);
            }
        }
    });
});
