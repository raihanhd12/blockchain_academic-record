...
describe("Sending QR Code to Student Executive Board in the Activity Credit Contract", function () {
    let SKKM, skkm, owner, student, studentExecutiveBoard, departmentStudentAssociation;
    let expect;
    ...
    beforeEach(async function () {
        ...
        await tx.wait();
        let bemUser = await skkm.users(studentExecutiveBoard.address);
        console.log(`Student Executive Board user added: ${JSON.stringify(bemUser)}`);
        expect(bemUser.exists).to.be.true;
        expect(bemUser.userAddress).to.equal(studentExecutiveBoard.address);

        // Submitting one Activity Credit for testing
        ...
        tx = await skkm.connect(departmentStudentAssociation).verifySKKM(0, true, "");
        await tx.wait();
    ...
    it("should send five QR Codes to the Student Executive Board", async function () {
        ...
        for (let i = 1; i <= 5; i++) {
            try {
                ...
                // Sending QR Code to the Student Executive Board
                console.log(`Sending QR Code ${i} to the Student Executive Board...`);
                ...
                console.error(`Failed to send QR Code ${i} to the Student Executive Board:`, error);
                throw new Error(`Failed to send QR Code ${i} to the Student Executive Board`);
            }
        }
    });
});
